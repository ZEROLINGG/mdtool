...
int coding_test() {
    ...
    try {
        // 测试文件编码检测
        auto encoding1 = detectFile(std::filesystem::path(R"(C:\Users\zerox\Desktop\test.txt)"));
        if (encoding1) {
            LOG_INFO(std::format("文件1编码: {}", *encoding1));
        } else {
            LOG_ERROR(std::format("检测文件1编码失败: {}", errorToString(encoding1.error())));
        }

        auto encoding2 = detectFile(std::filesystem::path(R"(C:\Users\zerox\Desktop\test2.txt)"));
        if (encoding2) {
            LOG_INFO(std::format("文件2编码: {}", *encoding2));
        } else {
            LOG_ERROR(std::format("检测文件2编码失败: {}", errorToString(encoding2.error())));
        }

        // 测试字符串编码检测
        const std::string testStr = R"""(path TEXT REQUIRED
                              单个md文档路径，或包含md文档的文件夹路径
  -l,     --language TEXT     指定代码语言
  -n,     --line UINT         指定行数
          --addl, --add-language Excludes: --updl --rmvl --delcl
                              为``` ```代码块添加语言(如果原本没有)
          --updl, --update-language Excludes: --addl --rmvl --delcl
                              强制更新``` ```代码块的语言
          --rmvl, --remove-language Excludes: --addl --updl --delcl)""";

        auto encoding3 = detectString(testStr);
        if (encoding3) {
            LOG_INFO(std::format("字符串编码: {}", *encoding3));
        } else {
            LOG_ERROR(std::format("检测字符串编码失败: {}", errorToString(encoding3.error())));
        }
        ...
        // 测试字符串转换
        std::string gbkString = "这是一个测试字符串";
        auto utf8Result = toUTF8(gbkString);
        if (utf8Result) {
            LOG_INFO("字符串已转换为UTF-8");
        } else {
            LOG_ERROR(std::format("字符串转换失败: {}", errorToString(utf8Result.error())));
        }

        // 测试文件转换
        auto fileResult = convertFile(
            R"(C:\Users\zerox\Desktop\test.txt)",
            R"(C:\Users\zerox\Desktop\test_utf8.txt)",
            "", // 自动检测源编码
            "UTF-8"
        );
        if (fileResult) {
            LOG_INFO("文件已转换为UTF-8");
        } else {
            LOG_ERROR(std::format("文件转换失败: {}", errorToString(fileResult.error())));
        }

        // 测试指定编码转换
        auto convertResult = convertString("测试内容", "GB18030", "UTF-8");
        if (convertResult) {
            LOG_INFO("GB18030 -> UTF-8 转换完成");
        } else {
            LOG_ERROR(std::format("编码转换失败: {}", errorToString(convertResult.error())));
        }

        // // 测试批量转换
        // auto batchResult = convertDirectory(
        //     R"(C:\Users\zerox\Desktop\input)",
        //     R"(C:\Users\zerox\Desktop\output)",
        //     "*.txt",
        //     "",
        //     "UTF-8",
        //     true
        // );
        // if (batchResult) {
        //     LOG_INFO(std::format("批量转换完成，处理了 {} 个文件", *batchResult));
        // } else {
        //     LOG_ERROR(std::format("批量转换失败: {}", errorToString(batchResult.error())));
        // }

    } catch (const std::exception& e) {
        LOG_ERROR(std::format("异常: {}", e.what()));
        return -1;
    }
